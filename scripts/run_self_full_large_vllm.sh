...
LAUNCH_LOG="${LAUNCH_LOG:-${LOG_DIR}/self-full-large-${RUN_TAG}.log}"
PROGRESS_LOG="${PROGRESS_LOG:-${LOG_DIR}/self-full-large-${RUN_TAG}.progress.log}"
...
verify_prompt_selection() {
    if [[ -z "${PROMPT_ID_FILE}" ]]; then
        "${PYTHON_BIN}" - <<'PY'
import json
...
pairs = [
    json.loads(line)
    for line in Path("data/processed/meta_eval_pairs.jsonl").read_text(encoding="utf-8").splitlines()
    if line.strip()
]
prompt_count = len({row["prompt_id"] for row in pairs})
print(f"Verified full prompt set: {prompt_count} prompts")
PY
        return
    fi
...
if not ids:
    raise SystemExit(f"Prompt id file is empty: {path}")
...
output_path() {
    local family="$1"
    local method="$2"
    if [[ -n "${OUTPUT_SPLIT}" ]]; then
        printf '%s/data/processed/rubrics_%s__%s__%s.jsonl' "${REPO_ROOT}" "${method}" "${family}" "${OUTPUT_SPLIT}"
    else
        printf '%s/data/processed/rubrics_%s__%s.jsonl' "${REPO_ROOT}" "${method}" "${family}"
    fi
}
...
progress_path() {
    local family="$1"
    local method="$2"
    printf '%s.progress.json' "$(output_path "${family}" "${method}")"
}
...
remove_outputs() {
    ...
    local method="$2"
    rm -f \
        "$(output_path "${family}" "${method}")" \
        "$(progress_path "${family}" "${method}")"
}
...
pairs = [
    json.loads(line)
    for line in Path(${REPO_ROOT@Q}).joinpath("data/processed/meta_eval_pairs.jsonl").read_text(encoding="utf-8").splitlines()
    if line.strip()
]
if ${PROMPT_ID_FILE@Q}:
    prompt_ids = {
...
expected = len({row["prompt_id"] for row in pairs})

path = Path(${REPO_ROOT@Q}) / "data/processed" / (
    f"rubrics_${method}__${family}__${OUTPUT_SPLIT}.jsonl"
    if ${OUTPUT_SPLIT@Q}
    else f"rubrics_${method}__${family}.jsonl"
)
if not path.exists():
    raise SystemExit(f"Missing output file: {path}")
row_count = sum(1 for line in path.open(encoding="utf-8") if line.strip())
if row_count != expected:
    raise SystemExit(f"Expected {expected} rows in {path}, found {row_count}")
...
start_server() {
    ...
    local server_log="${LOG_DIR}/${family}-self-full-${RUN_TAG}.server.log"
...
run_generation() {
    ...
    if [[ "${RESET_OUTPUTS}" == "1" ]]; then
        status "Resetting outputs family=${family} method=${method}"
        remove_outputs "${family}" "${method}"
    fi
    ...
    if [[ -n "${PROMPT_ID_FILE}" ]]; then
        cmd+=(--prompt-id-file "${PROMPT_ID_FILE}")
    fi
    if [[ -n "${OUTPUT_SPLIT}" ]]; then
        cmd+=(--output-split "${OUTPUT_SPLIT}")
    fi
...
status "Preparing prompt-only self rubric generation for full prompt set"
status "qwen_large -> GPUs ${QWEN_CUDA_DEVICES}; gemma_large -> GPUs ${GEMMA_CUDA_DEVICES}"
status "Config=${CONFIG_PATH} prompt_ids=${PROMPT_ID_FILE:-[all]} output_split=${OUTPUT_SPLIT:-[default]}"
